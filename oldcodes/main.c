...
void init_uart(void)
{
    ...
    UCA1CTL1 |= UCSWRST;
    UCA1CTL1 = UCSSEL__ACLK | UCSWRST;   // ACLK = 32kHz

    UCA1BR0 = 3;      // 9600 baud @ 32768 Hz
    UCA1BR1 = 0;
    UCA1MCTL = UCBRS_3;
...
{
    lcd_cmd(0xC0 + pass_index);   // Line-2 position
    lcd_data(c);
...
void handle_password_input(void)
{
    ...
    {
        pass_buf[pass_index-6] = uart_rx_char;
        lcd_show_pass_char(uart_rx_char);   // show digit on LCD
        pass_index++;
